...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Detect elevator/lift-shaft candidates.")
    ...
    parser.add_argument("--out-image", type=Path, default=ROOT / "outputs" / "annotated" / "elevators.png")
    parser.add_argument("--out-json", type=Path, default=ROOT / "outputs" / "json" / "elevators.json")
    return parser.parse_args()


def main() -> int:
    args = parse_args()
    elements = detect_elevator_candidates(args.image, work_dir=args.work_dir)
    ...
    print(f"detected elevator candidates: {len(elements)}")
