...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Detect stair candidates from a floor plan image.")
    ...
    parser.add_argument("--out-image", type=Path, default=ROOT / "outputs" / "annotated" / "stairs.png")
    parser.add_argument("--out-json", type=Path, default=ROOT / "outputs" / "json" / "stairs.json")
    return parser.parse_args()


def main() -> int:
    args = parse_args()
    elements = detect_stair_candidates(args.image, work_dir=args.work_dir)
    ...
    print(f"detected stair candidates: {len(elements)}")
